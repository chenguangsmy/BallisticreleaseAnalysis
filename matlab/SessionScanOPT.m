...
%         %% 
%         % since ss4401, subject 1 moves the robot a little bit
       X_opt0 = [-0.223245132035985;  -0.138391835261516;  -2.064836654945342];
       X_wam0 = [-0.479002729321357;  -0.517709615588190;  -0.036925117706784];
       A = [0.838352775680942   0.023021164250047   0.552593074215176
            0.042978983935401   0.972444624346153  -0.026335425221663
            -0.617184269703696  -0.013019931819001   0.764370446838361];
...
%         ];
    ...
    methods
        function obj = SessionScanOPT(ss_num, fname_other)
            ...
            % decide the matrix 
            if ss_num<4296
                obj.X_opt0 =    [-0.039926249838378;-0.120462853038858;-1.526446107588706];
                obj.X_wam0 =    [-0.484880470027251;-0.515017776586255;0.001370295584099];
                obj.A =    [  -0.037848635084157   1.006139173669417  -0.049302507925071
                              -0.925253905845084   0.004383012560907   0.338479651807431
                               0.382639418084085  -0.005081861415701   0.919003503546431]; 
            elseif ss_num<4379
                obj.X_opt0 = [ -0.207025909363952;  -0.159552961960486;  -1.672379585727084];
                obj.X_wam0 = [ -0.509554118480000;  -0.503134221202000;  -0.155901906928000];
                obj.A = [   -0.046020953751129   1.000420282996839  -0.075813059226936
                            -0.893388205043085   0.000259334777461   0.430759897297924
                            0.417832205169391  -0.015157199405532   0.850685498567284];
            else 
                obj.X_opt0 = [-0.223245132035985;  -0.138391835261516;  -2.064836654945342];
                obj.X_wam0 = [-0.479002729321357;  -0.517709615588190;  -0.036925117706784];
                obj.A = [0.838352775680942   0.023021164250047   0.552593074215176
                        0.042978983935401   0.972444624346153  -0.026335425221663
                        -0.617184269703696  -0.013019931819001   0.764370446838361];
            end
...
%                             end
                              ...
                              if isempty(datah.x{trial_idx})
                                  ...
                                  for marker_i = 1:4 % only 3 marker used
                                      try
                                        dataH_allm(:,:,marker_i) = datah.x{trial_idx}{marker_i};
                                      catch 
                                        dataH_allm(:,:,marker_i) = nan(size(datah.x{trial_idx}{1}));
...
%                             end
            ...
            if sum(obj.ss_num == [4404 4405])
                swap_seq = [1 3 2 4 5 6 7 8 9 10];
                obj.datah.x = obj.datah.x(swap_seq, :);
                obj.datah.y = obj.datah.y(swap_seq, :);
                obj.datah.z = obj.datah.z(swap_seq, :);
            end
