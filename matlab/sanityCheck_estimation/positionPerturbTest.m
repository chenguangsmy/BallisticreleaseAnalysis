...
% num = num; % unknown???
% den = den;
num = [M_parm B_parm K_parm];
den = 1;


G=tf(num,den) 
% create a step position pulse, whos derivative is gaussian
Fs = 2000;
t = linspace(0, 1, Fs);
dUgus = gaussmf(t,[0.015 0.05])/75.1309/100;
Ugus = cumsum(dUgus);
...
% % plot the displacement and corresponding force 
figure(); 
axh(1) = subplot(2,1,1);
plot(t, Ugus); 
xlabel('s'); 
