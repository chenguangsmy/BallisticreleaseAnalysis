function results = sys_id_pp(Data,idx_t,time_t,subj)
...
for f_sel= 1:3
    for d_sel = 1:3
        ...
        trial_l_up = length(Data(subj,1,f_sel,d_sel,:,unpert));
        ...
        time_new = linspace(0,2.25,1e3*2.25);
        for i = 1:trial_l_up
            if size(Data{subj,1,f_sel,d_sel,i,unpert}) == [1 1]
                ...
                %Replace final NaNs with last available experimental value
                %(this is needed to have all test with same exact length)
                A = force_interp(i,:);
                B = disp_interp(i,:);
                A(isnan(A)) = Data{subj,1,f_sel,d_sel,i,1}.f(1,idx_t{subj,1,f_sel,d_sel,i,1}(end));
                B(isnan(B)) = Data{subj,1,f_sel,d_sel,i,1}.ox(1,idx_t{subj,1,f_sel,d_sel,i,1}(end));
                force_interp(i,:) = A;
                disp_interp(i,:) = B;
                clear A B

                %Identification on Single Trial
                force_interp_t(i,:) = -(force_interp(i,:)-mean(force_interp(i,1:1100)));
                disp_interp_t(i,:) = disp_interp(i,:)-mean(disp_interp(i,1:1100));
                % SYS IDENT
                % Unperturbed
        ...
        force_up_avg_t =-(force_up_avg-mean(force_up_avg(1:1100)));
        disp_up_avg = mean(disp_interp);
        disp_up_avg_t = disp_up_avg-mean(disp_up_avg(1:1100));
        ...
        time_new_long = linspace(0,2.25,1e3*2.25);
        for pert = 2:3%2:length(Data(1,1,1,1,1,:))
            ...
            for i = 1:trial_l
                    if size(Data{subj,1,f_sel,d_sel,i,pert}) == [1 1]
                        ...
                        %Replace final NaNs with last available experimental value
                        %(this is needed to have all test with same exact length)
                        A = forcep_interp_ph(i,:);
                        B = dispp_interp_ph(i,:);
                        C = force_command_ph(i,:);
                        A(isnan(A)) = Data{subj,1,f_sel,d_sel,i,pert}.f(1,idx_t{subj,1,f_sel,d_sel,i,pert}(end));
                        B(isnan(B)) = Data{subj,1,f_sel,d_sel,i,pert}.ox(1,idx_t{subj,1,f_sel,d_sel,i,pert}(end));
                        C(isnan(C)) = Data{subj,1,f_sel,d_sel,i,pert}.Fp(1,idx_t{subj,1,f_sel,d_sel,i,pert}(end));
                        forcep_interp_ph(i,:) = A;
                        dispp_interp_ph(i,:) = B;
                        force_command_ph(i,:) = C;
                        clear A B C
                        ...
                        forcep_interp_al(i,:) = forcep_interp_ph(i,idx_fc(i)-ni:idx_fc(i)+nf);
                        dispp_interp_al(i,:) = dispp_interp_ph(i,idx_fc(i)-ni:idx_fc(i)+nf);
                        force_command_al(i,:) = force_command_ph(i,idx_fc(i)-ni:idx_fc(i)+nf);                        
                    end
            end
            ...
            for wind = 30:10:300 %Searching for Best Window
                ...
                for i = 1:trial_l
                    if size(Data{subj,1,f_sel,d_sel,i,pert}) == [1 1]
                        ...
                        % Setting Transfer Function a-priori Structure and blocking mass parameter
                        init_sys = idtf([1],[results.M_up(f_sel,d_sel) results.B_up(f_sel,d_sel) results.K_up(f_sel,d_sel)]);
                        init_sys.Structure.Denominator.Minimum = [0.9*results.M_up(f_sel,d_sel),0.1,0.1];
                        init_sys.Structure.Denominator.Maximum = [1.1*results.M_up(f_sel,d_sel),Inf,Inf];
                        opt = tfestOptions('SearchMethod','auto','InitializeMethod','iv','Display','off');
                        opt.InitializeOptions.MaxIterations = 60;
                        opt.InitialCondition = 'zero';
                        opt.InitializeOptions.Tolerance = 1e-3;
                        sysPt{i,idxopt} = tfest(data_est_Pt{i,idxopt},init_sys,opt); %Sys-ID with known mass                 
                ...
                opt.InitialCondition = 'zero';
                opt.InitializeOptions.Tolerance = 1e-3;
                sysP{idxopt} = tfest(data_est_P{idxopt},init_sys,opt); % Sys-ID with known mass
%                 sysP{idxopt} = tfest(data_est_P{idxopt},2,0);
            ...
            clear forcep_interp_al dispp_interp_al force_command_al
        end
