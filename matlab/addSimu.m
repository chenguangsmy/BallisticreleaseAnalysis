...
%% another figure describing df/dx
figure('Unit', 'inch', 'position', [0 8 6 5]); hold on;
...
for i = length(mass_list):-1:1
        ...
        plot(time, fce, 'color', colors(i,:));
        subplot(3,1,2); hold on;
        dx = pos - mean(pos(time>1.8 & time<2)); 
        plot(time, dx, 'color', colors(i,:));
        legend_arr{i} = ['M_r/(M_s+M_r)=' num2str(mass_list(i)) ];
        subplot(3,1,3); hold on;
        plot(time, -fce./dx, 'color', colors(i,:)); 
end

legend(legend_arr);
%ylim([-10, 21]); % force
subplot(3,1,1); yticks([-3:5]*4);
xlim([-0.1, 2]);
ylabel('dF (N)')
title('simulation on ballistic release');
set(gca, 'Ygrid', 'on');
subplot(3,1,2); yticks([-5:5]*0.02);
xlim([-0.1, 2]);
ylabel('dx (m)')
set(gca, 'Ygrid', 'on');
subplot(3,1,3); 
...
%% simulate with a shorter gaussian step perturbation 
% gaussian wave 
freq = 500;
dur = 8;
pert_t = 1;
...
for i = 1:5
   ...
   % damping = 20;
    ...
    xr0 = fce/stiffness0;
    simout{1}(i)=sim('/Users/cleave/Documents/projPitt/BallisticreleaseAnalysis/ballisticReleaseSimu/ballisticRelease_GaussianPerte.slx',...
            'FixedStep','0.002');  
end
%% % plot the gaussian perturbation
figure();
for ploti = 1:5
% for ploti = 1
%    axhf(ploti) = figure(ploti); hold on;
    ...
    axh(1) = subplot(3,1,1); title('force');    hold on;
    axh(2) = subplot(3,1,2); title('position'); hold on;
    axh(3) = subplot(3,1,3); title('velocity'); hold on; %title('dF / dx');  hold on;
    % in one plot
    for i = 1
        pos = simtmp(i).pos.Data;
        postime= simtmp(i).tout;
        ...
        %df_dx = (fce - force_list) ./ (pos - pos0);
        %plot(axh(3), time, df_dx, 'color', colors(i,:));
        plot(axh(3), time, vel, 'color', colors(i,:));
    end
    linkaxes(axh, 'x');
    %legend(legend_arr);
    subplot(axh(1));
...
pert_t_list = [4.05 4.05:0.2:4.95];
for perti = 1:6
...
% for i = 1:5
   ...
   % damping = 20;
    damping = damping_list(i);
    xr0 = fce/stiffness0;
    simout{1}(perti)=sim('/Users/cleave/Documents/projPitt/BallisticreleaseAnalysis/ballisticReleaseSimu/ballisticRelease_GaussianPerte.slx',...
            'FixedStep','0.002');  
% end
end
...
%    axhf(ploti) = figure(ploti); hold on;
    ...
    for i = 1
        ...
        legend_arr{i} = ['M_r/(M_s+M_r)=' num2str(mass_list(i)) ];
        plot(axh(2), time, pos, 'color', colors(coli,:));
        
        %df_dx = (fce - force_list) ./ (pos - pos0);
        %plot(axh(3), time, df_dx, 'color', colors(i,:));
        plot(axh(3), time, vel, 'color', colors(coli,:));
        coli = coli + 1;
    end
    linkaxes(axh, 'x');
    %legend(legend_arr);
...
%% % plot the gaussian perturbation, both origin and the subtraction over un-perturbed 
...
%    axhf(ploti) = figure(ploti); hold on;
    ...
    % in one plot
        ...
        plot(axh(2), time, fce - fce_nominal, 'color', colors(coli,:));
        legend_arr{i} = ['M_r/(M_s+M_r)=' num2str(mass_list(i)) ];
        plot(axh(4), time, pos - pos_nominal, 'color', colors(coli,:));
        
        plot(axh(6), time, vel - vel_nominal, 'color', colors(coli,:));    
