...
% This read-out is erroneous. This could possiblely because the bearing is
% taking the force from the force transducer. 
...
%     dF = sqrt(sum((Favg_load - Favg_unload).^2))
    dF = vecnorm(Favg_load - Favg_unload);
    F_measures(epoc_i) = dF;
...
%% multiple sessions 
% -y direction, without bearings
% I did this by rotating the mound direction of the Force Transducer. 
mass_list = [200 500 700 966 1166 1395 1595];
...
%% multiple sessions 
% -z direction (world direction). I tested it by directly handing the mass
% standards on the force transducer. 
...
xlabel('known weight (g)');
ylabel('measured weight (g)');
title('force transducer measure known mass Z');

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% Do the same thing here, however, plot the Y and Z direction in one figure 
...
% plot 
fh = figure('unit', 'inch', 'position',[0 0 5 2.5]); hold on;
axh(1) = subplot(1,2,1); hold on;
x = ones(6,1)*mass_list;
y = F_measuresC';
lh1 = scatter(axh(1),x(:), y(:),  10); 
% refline();
lh2 = plot(axh(1), [0 2500], [0 2500], 'Color', [0.8 0.8 0.8], 'LineWidth', 1); 
xlim([0 2500]); ylim([0 2500]);

legend([lh1, lh2], {'measured', 'known'});
xlabel('known weight (g)');
ylabel('measured weight (g)');
title('direction -Y');
...
F_measuresC = zeros(length(fnames), 6); % 6 epocs
fh_tmp = figure('position', [0 0 300 300]);
for file_i = 1:length(fnames)
fttmp = SessionScanFT(3957,fnames{file_i});
fttmp.plotForceOrigin()
...
lh1 = scatter(x(:), y(:),  10); 
% refline();
lh2 = plot(axh(2), [0 2500], [0 2500], 'Color', [0.8 0.8 0.8], 'LineWidth', 1); 
xlim([0 2500]); ylim([0 2500]);
...
ylabel('measured weight (g)');
title('direction +Z');
sgtitle('force measurement on different directions');
saveas(fh, '/Users/cleave/Documents/projPitt/BallisticreleaseAnalysis/matlab/sanityCheck_StiffnessMeasurement/FT_sanityCheck/FT_measurements.png');