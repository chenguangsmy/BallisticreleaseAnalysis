...
%             this.get_2DSim_linear();
            this.get_2DSim_WAM_SISO(); % FIX: Change hand inertia       

%             this.get_2DSim_WAM_MIMO(); % FIX: Change hand inertia    
%             
%             this.get_InMotionConstant();
%             this.get_InMotionTurning();
...
%             axes(ax4); view(35,25); title(22); xlim([0 M2_tan/this.sfrq]); zlim([-0.1 0.1]); xlabel('lag (s)'); ylabel('Time (s)'); zlabel('\Delta e_t (m)'); grid on; set(gca,'fontsize',fs);
        ...
        function [] = get_InMotionConstant(this)
            
            %             file_bot = ['/Users/jhermus/Documents/School/MIT/Research/Limb_Impedance/PreliminaryData/ensambleTestData/Gaussian_1N_handel_15min_FPGA-17-Nov-2020-18-02.mat'];
            %             file_bot = ['/Users/jhermus/Documents/School/MIT/Research/Limb_Impedance/PreliminaryData/ensambleTestData/Point_center_5min_FPGA-14-Dec-2020-19-59.mat'];
            
            %% Subtact
            file_bot = ['data/Point_center_5min_FPGA-14-Dec-2020-19-59.mat'];
            file_bot_hand = ['data/Point_center_hand_5min_FPGA-14-Dec-2020-20-50.mat'];
            
            %Test 50 Hz butterworth filter
            %             file_bot = ['/Users/jhermus/Documents/School/MIT/Research/Limb_Impedance/PreliminaryData/ensambleTestData/Point_center_5min_50Hz_FPGA-21-Dec-2020-13-38.mat'];
            
            %
            [z_r, u_r, N, L, R, M1, M2] = this.importData('fixedAmplitudePRBS',file_bot);
            [z_r, z_r, u_r, u_r, N, L, R, M1, M2] = get_lowerSamplingRate(this,1, z_r, z_r, u_r, u_r, N, L, R, M1, M2);

            [h_hat_MA_bot, MAwindow] = this.ensambleSysID(z_r, u_r, N, L, R, M1, M2);

            cf = 50; % cutoff freqnency
            [b,a] = butter(2,cf/(this.sfrq/2)); % make filter
            h_hat_MA_bot_filt = zeros(L,N);
            dexNonZero = M2-M1+1+MAwindow/2:N+M1-MAwindow/2;
            for i = dexNonZero
                h_hat_MA_bot_filt(:,i) = filtfilt(b,a,h_hat_MA_bot(:,i)); % apply fitler
            end
            [h_model_bot, M_bot, B_bot, K_bot,iDex,VAFirf_bot] = this.fitModel(h_hat_MA_bot_filt, z_r, u_r, N, L, R, M1, M2);
            
            [z_r, u_r, N, L, R, M1, M2] = this.importData('fixedAmplitudePRBS',file_bot_hand);
            [h_hat_MA_bot_hand, MAwindow] = this.ensambleSysID(z_r, u_r, N, L, R, M1, M2);
            h_hat_MA_bot_hand_filt = zeros(L,N);
            for i = dexNonZero
                h_hat_MA_bot_hand_filt(:,i) = filtfilt(b,a,h_hat_MA_bot_hand(:,i)); % apply fitler
            end
            [h_model_bot_hand, M_bot_hand, B_bot_hand, K_bot_hand,iDex,VAFirf_bot_hand] = this.fitModel(h_hat_MA_bot_hand_filt, z_r, u_r, N, L, R, M1, M2);
            %
            M_hand = M_bot_hand-M_bot;
            B_hand = B_bot_hand-B_bot;
            K_hand = K_bot_hand-K_bot;
            
            iDex = iDex(1:end-1);
            figure('Position',[440 274 560 524]);
            ax1 = subplot(3,1,1);
            plot(this.tvec(iDex), M_hand(iDex),'.-b','linewidth',2.5,'markersize',15); hold on;
            plot(this.tvec(iDex), M_bot(iDex),'--k','linewidth',2.5,'markersize',15);
            plot(this.tvec(iDex), M_bot_hand(iDex),'--r','linewidth',2.5,'markersize',15);
            grid on;  xlim([0.2 this.tvec(end)]); ylabel('Mass (kg)'); %ylim([0 4]);
            
            ax2 = subplot(3,1,2);
            plot(this.tvec(iDex), B_hand(iDex),'.-b','linewidth',2.5,'markersize',15); hold on;
            plot(this.tvec(iDex), B_bot(iDex),'--k','linewidth',2.5,'markersize',15);
            plot(this.tvec(iDex), B_bot_hand(iDex),'--r','linewidth',2.5,'markersize',15);
            grid on;  xlim([0.2 this.tvec(end)]); ylabel('Damping (Ns/m)'); ylim([0 100]); yticks([0:20:100]);
            
            ax3 = subplot(3,1,3);
            plot(this.tvec(iDex), K_hand(iDex),'.-b','linewidth',2.5,'markersize',15); hold on;
            plot(this.tvec(iDex), K_bot(iDex),'--k','linewidth',2.5,'markersize',15);
            plot(this.tvec(iDex), K_bot_hand(iDex),'--r','linewidth',2.5,'markersize',15);
            grid on;  xlim([0.2 this.tvec(end)]); ylabel(' Stiffness (N/m)'); ylim([0 7000]); yticks([0:2000:7000]);
            xlabel('Time (s)'); set([ax1,ax2,ax3],'fontsize',18);
            legend('Hand','InMotion','InMotion+Hand');legend boxoff;
            
            figure;
            plot(this.tvec(iDex), VAFirf_bot(iDex),'--k','linewidth',2.5,'markersize',15); hold on;
            plot(this.tvec(iDex), VAFirf_bot_hand(iDex),'--r','linewidth',2.5,'markersize',15);
            grid on;  xlim([0.2 this.tvec(end)]); ylabel(' VAF_{irf}'); ylim([0 100]);
            xlabel('Time (s)'); set(gca,'fontsize',18);
            
            % Average
            [h_hat_MA, MAwindow] = this.ensambleSysID(z_r, u_r, N, L, R, M1, M2);
            %             [h_hat_MA_window, MAwindow] = this.ensambleSysID_windowed(z_r, u_r, N, L, R, M1, M2);
            
            % add processing for InMotion data
            dexNonZero = M2-M1+1+MAwindow/2:N+M1-MAwindow/2;
            figure;
            plot(this.tvec(1:L),h_hat_MA(:,dexNonZero)); hold on; grid on;
            plot(this.tvec(1:L),mean(h_hat_MA(:,dexNonZero),2),'-k','linewidth',2.5);
            xlabel('lag (s)'); ylabel('x (m)');
            set(gca,'fontsize',18); grid on;
            ...
            figure;
            for i = M2+1:50:N+M1
                plot3(this.tvec(1:L),i*ones(1,L),h_hat_MA_filt(:,i),'-','linewidth',2); hold on;
            end
            xlabel('lag (s)');
            ylabel('Time (sec)');
        ...
        function [] = get_InMotionTurning(this)
            
            %% Import data
                            %Old trial with bionary random sequency which is either 6 or -6
                pathh = '/Users/jhermus/Documents/School/MIT/Research/InMotion2/hermus/ensambleMethod/data/';
                
%                   fileNames = {'nePret4_FPGA-22-Apr-2021-10-58.mat'};

                % Normal tangential preturbations
                fileNames = {'Test1_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test2_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test3_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test4_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test5_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test6_ne1N_10min_FPGA-12-May-2021.mat',...
                             'Test1_ne1N_12min_FPGA-22-Apr-2021-13-25.mat'};

                % Carteasion preturbations in xy
%                 fileNames = {'Test1_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test2_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test3_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test4_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test5_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test6_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test7_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat',...
%                              'Test8_10Min_1NUniform_CCW_FPGA-14-Apr-2021.mat'};
%               fileNames = {'Gaussian_2N_turning_10min_trial1_FPGA-17-Nov-2020-19-04.mat',...
%                            'Gaussian_2N_turning_10min_trial2_FPGA-17-Nov-2020-19-34.mat',...
%                            'Gaussian_2N_turning_10min_trial3_FPGA-17-Nov-2020-19-34.mat'};
%                 fileNames = {'Turning_Medium_5min_trial2_FPGA-14-Dec-2020-22-10.mat',...
%                              'Turning_Medium_5min_trial3_FPGA-14-Dec-2020-22-10.mat'};
                %                 % Wrong Direction
                %                 'Turning_Medium_5min_trial1_FPGA-14-Dec-2020-22-10.mat',...
                
                %                  fileNames = {'Uniform_4N_10min_mediumSpeed_turning_trial1_FPGA-20-Nov-2020-18-37'};
                
                ensambleLength = 13.33;
                ...
                % Import multiple trials
                %               [z_r, u_r, R] = this.importMultiHumanTrials(pathh,fileNames);
                
                % Import multiple trials and apply time scaling
                [z1_r,z2_r, u1_r, u2_r, R] = this.importMultiHumanTrials_TimeScale(pathh,fileNames,ensambleLength);
                
                % Change sampling rate to 500 Hz
                [z1_r, z2_r, u1_r, u2_r, N, L, R, M1, M2] = get_lowerSamplingRate(this,ensambleLength, z1_r, z2_r, u1_r, u2_r, N, L, R, M1, M2);
                
                % Temp fix for end wrap
                z1_r(:,end) = 0;
                z2_r(:,1) = 0;
                z2_r(:,end-3:end) = ones(size(z2_r,1),4).*z2_r(:,end-4);
                
                figure;
                subplot(2,1,1); 
                plot(this.tvec,z1_r'); hold on; xlim([0 this.tvec(end)]);
                plot(this.tvec,mean(z1_r),'-k','linewidth',2.5); %ylim([0.095 0.105]);
                xlabel('Time (s)'); ylabel('x (m)');
                set(gca,'fontsize',18); grid on;
                
                subplot(2,1,2); 
                plot(this.tvec,z2_r'); hold on; xlim([0 this.tvec(end)]);
                plot(this.tvec,mean(z2_r),'-k','linewidth',2.5); %ylim([0.095 0.105]);
                xlabel('Time (s)'); ylabel('x (m)');
                set(gca,'fontsize',18); grid on;
                ...
                for i = 1:R
                    z1_r(i,:) = z1_r(i,:) - z1_r_mean;
                    z2_r(i,:) = z2_r(i,:) - z2_r_mean;
                end
                
                figure;
                subplot(2,1,1);
                plot(this.tvec,z1_r'); hold on; %xlim([0 13]);
                xlabel('Time (s)'); ylabel('x (m)'); ylim([-0.0050 0.0050]);
                set(gca,'fontsize',18); grid on;
                
                subplot(2,1,2);
                plot(this.tvec,z2_r'); hold on; %xlim([0 13]);
                xlabel('Time (s)'); ylabel('x (m)'); %ylim([-0.0050 0.0050]);
                set(gca,'fontsize',18); grid on;
                
                % Take out extream tangental cases
                dex = [];
                for i = 1:R
                    if(max(abs(z2_r(i,:))) < 0.045)
                        dex = [dex;i];
                    end 
                end
            ...
            %% Turning Data
%             [z_r, u_r, N, L, R, M1, M2] = this.importData('humanTurnData');
%             [h_hat_MA, MAwindow] = this.ensambleSysID(z1_r, u1_r, N, L, R, M1, M2);
%             cf = 50; % cutoff freqnency
%             [b,a] = butter(2,cf/(this.sfrq/2)); % make filter
%             h_hat_MA_filt = zeros(L,N);
%             dexNonZero = M2-M1+1+MAwindow/2:N+M1-MAwindow/2;
...
%                 h_hat_MA_filt(:,i) = filtfilt(b,a,h_hat_MA(:,i)); % apply fitler
%             end
%             [h_model, M, B, K,iDex,VAFirf] = this.fitModel(h_hat_MA_filt, z_r, u_r, N, L, R, M1, M2);
%             disp('test');

            %% Test MIMO
            
            [H_hat_MA, MAwindow] = this.ensambleSysID_Matrix(z1_r, z2_r, u1_r, u2_r, N, L, R, M1, M2);
            h_hat_ne11 = H_hat_MA(:,:,1);
            h_hat_ne12 = H_hat_MA(:,:,2);
            h_hat_ne21 = H_hat_MA(:,:,3);
            h_hat_ne22 = H_hat_MA(:,:,4);
            
            iDex = floor(linspace(M2+1+40,N+M1-40,15)); 
%           iDex = M2+1+40:N+M1-40;
            ...
            % Optimization fit
            %             [h_model_ne11, M_ne11, B_ne11, K_ne11,VAFirf_ne11] = this.fitModel(h_hat_ne11_filt, N,L, M1, M2, iDex);
            %             [h_model_ne12, M_ne12, B_ne12, K_ne12,VAFirf_ne12] = this.fitModel(h_hat_ne12, z_r_ne12, u_r_ne12, N,L_norm, R, M1, M2_norm, iDex);
            %             [h_model_ne21, M_ne21, B_ne21, K_ne21,VAFirf_ne21] = this.fitModel(h_hat_ne21, z_r_ne21, u_r_ne21, N,L_tan, R, M1, M2_tan, iDex);
            %             [h_model_ne22, M_ne22, B_ne22, K_ne22,VAFirf_ne22] = this.fitModel(h_hat_ne22, z_r_ne22, u_r_ne22, N,L_tan, R, M1, M2_tan, iDex);
            ...
            % Fit Model Prony
            [K_ne_hat, h_model_ne11, h_model_ne12, h_model_ne21, h_model_ne22, VAFirf] = get_MIMO_Prony(this,iDex, h_hat_ne11_filt, h_hat_ne12_filt, h_hat_ne21_filt, h_hat_ne22_filt, M2);
            
            %% Make fit plots 
            K_ne_hat(1,1,:) =  K_ne_hat(1,1,:)-2500;
%             B_ne11 = B_ne11-40;
                        
            turnPeriod = ensambleLength;
            test = simSystem2D(turnPeriod);
            thcp = z2_r_mean./test.lc;
                            
%             F_contact = [mean(f_r_ne1); zeros(1,N)]; % Add tangential if robot dynamics are added
%             figure; plot(F_contact');
...
%             figure;
            ...
            for i = iDex
                ...
                X = [test.lc*cos(thcp(i))' - test.d1; test.d2 - test.lc*sin(thcp(i)+pi)'];
                [tmp,ee,nn] = test.get_crankPostion(X,'shoulder');
                [q_1, q_2] = test.invKino(X(1),X(2));
                J = test.get_jacobian([q_1;q_2]);
                M_q = test.get_massMatrixArm([q_1;q_2]);
                ...
                crank_x = [test.d1, test.d1 + test.lc*cos(thcp)];
                crank_y = [test.d2, test.d2 + test.lc*sin(thcp)];                
                
                
%                 ax = plot(armPosVec_X, armPosVec_Y, '-ob',...
%                     crank_x,     crank_y,     '--k',...
%                     X(1)+0.1*[0,ee(1)],X(2)+0.1*[0,ee(2)],':b',...
%                     X(1)+0.1*[0,nn(1)],X(2)+0.1*[0,nn(2)],':r','linewidth',3);
%                 ylim([0 1.2]); xlim([-0.56 0.56]); grid on; axis equal;
%                 ylabel('Y Distance (m)','fontsize',14);
%                 xlabel('X Distance (m)','fontsize',14);
% %                 legend('Arm','Zero-Force Trajectory','Crank','Constraint Path','Robot','location','northwest'); % Add ZFT
...
%                 K_ne_hat(:,:,i) = K_ne_hat(:,:,i) - [nn,ee]'*( parJ_parq1(:,:,i)*F_contact(1,i) + parJ_parq2(:,:,i)*F_contact(2,i))*[nn,ee];
...
%                 M_q_hat(:,:,i) = J'*squeeze(M_xy_hat(:,:,i))*J;
                ...
                Z_true_model = M_ne_const(:,:,i)*s^2 + [B_ne_const(:,:,i)+[40,0;0,0]]*s + [K_ne_const(:,:,i)+[2500,0;0,0]]; % Includes robot
...
%                 pause(0.01);
            ...
            for i = iDex
                VAF_true(1,i) = get_VAF(this,h_true_model(:,1,1,i),h_hat_ne11_filt(:,i));
                VAF_true(2,i) = get_VAF(this,h_true_model(:,1,2,i),h_hat_ne12_filt(:,i));
                VAF_true(3,i) = get_VAF(this,h_true_model(:,2,1,i),h_hat_ne21_filt(:,i));
                VAF_true(4,i) = get_VAF(this,h_true_model(:,2,2,i),h_hat_ne22_filt(:,i));
            end
            ...
            for i = iDex %floor(linspace(iDex(1),iDex(end),15)) 
                axes(ax1);
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_hat_ne11_filt(:,i),'.r'); hold on;
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_model_ne11(:,i),'-b'); hold on;
%                 plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_true_model(:,1,1,i),'-k'); hold on;
                
                axes(ax2);
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_hat_ne12_filt(:,i),'.r'); hold on;
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_model_ne12(:,i),'-b'); hold on;
%                 plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_true_model(:,1,2,i),'-k'); hold on;

                
                axes(ax3);
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_hat_ne21_filt(:,i),'.r'); hold on;
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_model_ne21(:,i),'-b'); hold on;
%                 plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_true_model(:,2,1,i),'-k'); hold on;

                
                axes(ax4);
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_hat_ne22_filt(:,i),'.r'); hold on;
                plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_model_ne22(:,i),'-b'); hold on;
%                 plot3(this.tvec(1:L),this.tvec(i)*ones(1,L),h_true_model(:,2,2,i),'-k'); hold on;
            ...
            axes(ax4); view(35,25); title(22); xlim([0 M2/this.sfrq]); %zlim([-0.5 0.5]); 
            xlabel('lag (s)'); ylabel('Time (s)'); zlabel('\Delta e_t (m)'); grid on; set(gca,'fontsize',fs);
            
            i = iDex(10);
            figure;
            plot(this.tvec(1:L),h_hat_ne11_filt(:,i),'.r','markersize',20); hold on;
            plot(this.tvec(1:L),h_model_ne11(:,i),'-b','linewidth',2.5); hold on;
            plot(this.tvec(1:L),h_true_model(:,1,1,i),'--k','linewidth',2.5); hold on;
            xlabel('Lag (s)'); ylabel('IRF (m)'); grid on; set(gca,'fontsize',16);
            legend('Estimate','Fit Model','Known Model');
            ...
            plot(this.tvec(iDex),VAFirf(4,iDex),'k','linewidth',2.5);
            ylabel('VAF_{irf}'); xlabel('Time (s)'); set(gca, 'fontsize', 16); ylim([0 100]);
            legend('11','12','21','22');xlim([this.tvec(iDex(1)) this.tvec(iDex(end))]);
...
%             ax3 = subplot(3,1,3);
            p1=plot(this.tvec(iDex), squeeze(K_ne_hat(1,1,iDex)),'-b',this.tvec(iDex), squeeze(K_ne_const(1,1,iDex)),'--b','linewidth',2.5,'markersize',15); hold on;
            p2=plot(this.tvec(iDex), squeeze(K_ne_hat(1,2,iDex)),'-r',this.tvec(iDex), squeeze(K_ne_const(1,2,iDex)),'--r','linewidth',2.5,'markersize',15); hold on;
            p3=plot(this.tvec(iDex), squeeze(K_ne_hat(2,1,iDex)),'-g',this.tvec(iDex), squeeze(K_ne_const(2,1,iDex)),'--g','linewidth',2.5,'markersize',15); hold on;
            p4=plot(this.tvec(iDex), squeeze(K_ne_hat(2,2,iDex)),'-k',this.tvec(iDex), squeeze(K_ne_const(2,2,iDex)),'--k','linewidth',2.5,'markersize',15);
            grid on;  ylabel(' Stiffness (N/m)'); xlabel('Time (s)'); ylim([-2000 7000]); xlim([this.tvec(iDex(1)) this.tvec(iDex(end))]);
            set(gca,'fontsize',16);
...
%             subplot(3,1,3); 
            plot(this.tvec(iDex), squeeze(K_q_hat(1,1,iDex)),'-b',this.tvec(iDex),squeeze(K_q_const(1,1,iDex)),'--b',...
                this.tvec(iDex), squeeze(K_q_hat(1,2,iDex)),'-r',this.tvec(iDex),squeeze(K_q_const(1,2,iDex)),'--r',...
                this.tvec(iDex), squeeze(K_q_hat(2,1,iDex)),'-g',this.tvec(iDex),squeeze(K_q_const(2,1,iDex)),'--g',...
                this.tvec(iDex), squeeze(K_q_hat(2,2,iDex)),'-k',this.tvec(iDex),squeeze(K_q_const(2,2,iDex)),'--k','markersize',20,'linewidth',2.5);
            ylim([0 200]); xlim([this.tvec(iDex(1)) this.tvec(iDex(end))]); ylabel('Stiffness (Nm/rad)'); xlabel('Time (s)'); set(gca,'fontsize',18);
...
%             
% 
%             M = 0.5*this.sfrq;  % Choose to be even          
%             N = size(z2_r,2);
%             binDexCenter = (M/2+1):M:N-(M/2+1);
%             for i = 1:length(binDexCenter)
%                 binDex(:,i) = (1:M+1)+binDexCenter(i)-(M/2+1);
%             end
%             
%             for i = 1:length(binDexCenter)
%                 [TF_freq,Z11_l_mag(:,i),Z12_l_mag(:,i),Z21_l_mag(:,i),Z22_l_mag(:,i),Z11_l_phi(:,i),Z12_l_phi(:,i),Z21_l_phi(:,i),Z22_l_phi(:,i),PC11_s(:,i),PC12_s(:,i),PC21_s(:,i),PC22_s(:,i)] = get_Z_spectral_individual(this, binDex(:,i), z1_r, z2_r, u1_r, u2_r);
%             end
%             
%             
%             figure('position',[289 231 895 527]);
...
%             
%             for i = 1:length(binDexCenter)
%                 axes(ax1); plot3(TF_freq,this.tvec(binDexCenter(i))*ones(1,length(TF_freq)),Z11_l_mag(:,i),'.r'); hold on;
% %                 axes(ax2); plot3(TF_freq,this.tvec(binDex(i))*ones(1,length(TF_freq)),Z11_l_mag(:,i),'.r'); hold on;
% %                 axes(ax3); plot3(TF_freq,this.tvec(binDex(i))*ones(1,length(TF_freq)),Z22_l_mag(:,i),'.r'); hold on;
%                 axes(ax4); plot3(TF_freq,this.tvec(binDexCenter(i))*ones(1,length(TF_freq)),Z22_l_mag(:,i),'.r'); hold on;
%             end
%             fs = 15;
%             axes(ax1); set(ax1, 'ZScale', 'log','XScale','log'); %xlim([0 M2_norm/this.sfrq]); zlim([-0.011 0.011]); xlabel('lag (s)'); ylabel('Time (s)'); zlabel('\Delta e_n (m)'); grid on; set(gca,'fontsize',fs);
%             axes(ax4); set(ax4, 'ZScale', 'log','XScale','log'); %xlim([0 M2_tan/this.sfrq]); zlim([-0.011 0.011]); xlabel('lag (s)'); ylabel('Time (s)'); zlabel('\Delta e_n (m)'); grid on; set(gca,'fontsize',fs);
...
%             plot(TF_freq,PC11_s,'LineWidth',2);
...
%             set(gca,'fontWeight','bold','fontSize',12); hold on;
%             plot(TF_freq,PC12_s,'LineWidth',2); hold off;
...
%             set(gca,'fontWeight','bold','fontSize',12); hold on;
%             plot(TF_freq,PC21_s,'LineWidth',2); hold off;
%             grid on;box on;ylim([0 1]); %axis([xLowerLim xUpperLim 0 1]);
%             xlabel('Hz','fontWeight','bold','fontSize',14);
...
%             set(gca,'fontWeight','bold','fontSize',12); hold on;
%             plot(TF_freq,PC22_s,'LineWidth',2); hold off;
%             grid on;box on;ylim([0 1]); %axis([xLowerLim xUpperLim 0 1]);
%             xlabel('Hz','fontWeight','bold','fontSize',14);
...
% %% Sanity check for InMotion
% clear all
% close all
% clc
% 
% % Found problem: CORDIC sin function is periodic to 2 not 2pi
% % Also note that gamma_rad is shifted by pi/2 relative to thcp
% 
% N = 500;
% p = linspace(-pi, pi,N)';
% n = randn(N,1);
...
% 
% figure;
% subplot(2,1,1); plot(p,n,'*','markersize',5); hold on;
% subplot(2,1,2); plot(p,e,'*','markersize',5); hold on;
% 
% x = n.*cos(p) + e.*(-sin(p));
% y = n.*(sin(p)) + e.*(cos(p));
% 
% subplot(2,1,1); plot(p,x,'.','markersize',10); hold on;
% subplot(2,1,2); plot(p,y,'.','markersize',10); hold on;
% 
% n = x.*cos(p) + y.*(sin(p));
% e = x.*(-sin(p)) + y.*(cos(p));
% 
% subplot(2,1,1); plot(p,n,'.','markersize',10); 
% ylim([-1.5 1.5]); xlim([0 2*pi]); ylabel('dof 1'); 
% legend('n','x','x_{hat}'); set(gca,'fontsize',16);
% 
% subplot(2,1,2); plot(p,e,'.','markersize',10); 
% ylim([-1.5 1.5]); xlim([0 2*pi]); ylabel('dof 2'); 
% xlabel('p (rad)'); legend('e','y','e_{hat}'); set(gca,'fontsize',16);
% 
% load('data/nePretTest3-21-Apr-2021-17-10.mat');
% figure; plot(gamma_rad-pi/2);
% 
% figure;
% plot(posX_m,posY_m); hold on;
% plot(posX_m(1),posY_m(1),'o');
% plot([0,0.1*cos(gamma_rad(1)-pi/2)],[0,0.1*sin(gamma_rad(1)-pi/2)]);
